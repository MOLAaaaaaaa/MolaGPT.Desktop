...
public sealed partial class MessageViewModel : ObservableObject, IDisposable
{
    ...
    /// <summary>首个输出到上游结束的秒数，包含工具等待；未完成时为空。</summary>
    ...
    /// <summary>
    /// 优先使用 Provider 提供的生成速度；没有精确值时用本轮输出量和耗时估算。
    /// </summary>
    ...
    /// <summary>记录请求发出后的首个输出。正文、思考、工具卡任一先到都算。</summary>
    private void MarkFirstOutput()
    {
        if (_disposed || _requestStartedAt is null || _firstOutputAt is not null) return;
