...
public sealed partial class ConversationListViewModel : ObservableObject
{
    ...
    /// <summary>True while a just-deleted batch can still be undone — drives the
    /// sidebar's undo toast. Set false when the window expires or is consumed.</summary>
    [ObservableProperty] private bool _hasPendingUndo;
    [ObservableProperty] private string _pendingUndoMessage = string.Empty;
    [ObservableProperty]
    ...
    /// <summary>Arm the undo toast for a freshly soft-deleted batch. Only the most
    /// recent batch is undoable (last-action semantics); the previous window, if
    /// any, is discarded. Auto-expires after <see cref="UndoWindowMs"/>.</summary>
    private void BeginUndoWindow(IReadOnlyList<string> ids)
    {
        if (_repository is null || ids.Count == 0) return;

        lock (_undoGate)
        {
            _pendingUndoIds = ids.ToList();
            _undoTimer?.Dispose();
            _undoTimer = new System.Threading.Timer(
                _ => ExpireUndo(), null, UndoWindowMs, System.Threading.Timeout.Infinite);
        }

        PendingUndoMessage = ids.Count == 1 ? "已删除 1 个对话" : $"已删除 {ids.Count} 个对话";
        HasPendingUndo = true;
    }
    ...
    private void ExpireUndo()
    {
        lock (_undoGate)
        {
            _pendingUndoIds = null;
            _undoTimer?.Dispose();
            _undoTimer = null;
        }
        HasPendingUndo = false;
        PendingUndoMessage = string.Empty;
    }

    /// <summary>Restore the last soft-deleted batch. Reloads from SQLite so the
    /// rows return to their exact prior sort position, and re-highlights the
    /// previously active conversation (a no-op swap for the open chat). Gated by
    /// the toast's visibility (bound to <see cref="HasPendingUndo"/>) rather than
    /// CanExecute, so the timer thread never has to raise CanExecuteChanged.</summary>
    [RelayCommand]
    private void UndoDelete()
    {
        ...
        if (ids is null || ids.Count == 0 || _repository is null) return;

        var keepSelected = SelectedId;
        _repository.RestoreMany(ids);
        ApplyRows(_repository.ListActive());
        if (!string.IsNullOrEmpty(keepSelected) && FindItem(keepSelected) is not null)
    ...
    /// <summary>Drives the search box's clear (×) button visibility.</summary>
