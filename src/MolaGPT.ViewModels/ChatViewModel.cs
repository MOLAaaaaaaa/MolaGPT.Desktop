...
public sealed partial class ChatViewModel : ObservableObject
{
    ...
    /// <summary>
    /// Session-level artifacts: fences the answers produced (published by the
    /// transcript as it parses them) plus files in the conversation's Python
    /// working directory (rescanned via <see cref="RefreshArtifacts"/>).
    /// </summary>
    public ArtifactWorkspace ArtifactWorkspace { get; }

    public ObservableCollection<ArtifactItemViewModel> Artifacts => ArtifactWorkspace.Items;
    ...
    public event EventHandler<ArtifactActionEventArgs>? ArtifactActionRequested;

    /// <summary>
    /// Rescans the conversation's working directory. Cheap directory
    ...
    /// <summary>
    /// Canvas formats the user reads in the answer as ordinary code blocks
    /// rather than as chips. Pushed from settings; each fence can still be
    /// switched on its own in the transcript.
    /// </summary>
    [ObservableProperty] private IReadOnlySet<ArtifactRenderKind> _codeFenceKinds = new HashSet<ArtifactRenderKind>();

    /// <summary>Called by the transcript chip for the fence at <paramref name="ordinal"/>
    /// in <paramref name="message"/>.</summary>
    public void RequestArtifactAction(MessageViewModel message, int ordinal, ArtifactAction action)
    {
        var item = ArtifactWorkspace.FindItem(message, ordinal);
        if (item is null) return;
        var version = item.IndexOfVersion(message, ordinal);
        ArtifactActionRequested?.Invoke(this, new ArtifactActionEventArgs(item, version, action));
    }
        ...
        SettingsRepository? settingsRepo)
    {
        ...
        ArtifactWorkspace = new ArtifactWorkspace(() => _messages);
        ArtifactWorkspace.Changed += (_, _) =>
        {
    ...
    private static List<PreparedMessage> PrepareMessageSnapshot(IReadOnlyList<MessageRow> rows)
    {
        ...
        for (var index = 0; index < rows.Count; index++)
        {
            ...
            IReadOnlyList<ArtifactRefChip>? artifactRefs = null;
            ...
            if (!string.IsNullOrEmpty(row.Meta))
            {
                try
                {
                    ...
                    artifactRefs = ParseArtifactRefs(doc.RootElement);
    ...
    private static string BuildMessageMeta(MessageViewModel vm)
    {
        ...
        if (vm.ArtifactRefs is { Count: > 0 })
        {
            meta["artifact_refs"] = new JsonArray(vm.ArtifactRefs
    ...
    private static IReadOnlyList<ArtifactRefChip>? ParseArtifactRefs(JsonElement root)
    {
        if (!root.TryGetProperty("artifact_refs", out var node) || node.ValueKind != JsonValueKind.Array)
            return null;

        var list = new List<ArtifactRefChip>();
        foreach (var item in node.EnumerateArray())
        {
            if (item.ValueKind != JsonValueKind.Object) continue;
            var title = ReadOptionalString(item, "title");
            if (string.IsNullOrWhiteSpace(title)) continue;
            list.Add(new ArtifactRefChip(
                title!,
                ReadOptionalString(item, "kind") ?? string.Empty,
                ReadMetaInt(item, "version"),
                ReadMetaInt(item, "versions")));
        }
