...
它同时支持 MolaGPT 账号模式和 BYOK 模式。你可以直接登录 MolaGPT 账号使用平台模型、账号额度和同步能力，也可以接入自己的 OpenAI、Anthropic、DeepSeek、Gemini 或 OpenAI-compatible 服务。

相比 Web 端，Desktop 版本更强调本地工作流。它可以保存本地对话，管理 Provider，处理附件，调用工具，执行联网搜索，也可以通过 Work 模式读取指定工作目录，让模型真正理解你的项目、文档和文件结构。
...
另一类是 BYOK Provider。你可以添加自己的 API Key，并配置 OpenAI-compatible、Anthropic、Gemini 等接口。BYOK 模式下，请求会直接发送到你配置的服务端点。
...
在 Work 模式中，你可以为当前对话选择一个本地工作目录。模型可以在这个目录里读取文件、搜索文件、理解项目结构，并结合对话内容完成更贴近真实工作的任务。
...
Work 模式的文件工具默认遵循只读边界。模型可以读取文件、列出目录、glob 搜索路径、匹配文件内容，但不会直接修改你的原始文件。需要生成的新文件会进入独立的工作输出位置，避免覆盖用户的真实项目文件。

这个设计的目标很简单：让模型能看见你的工作现场，同时尽量控制本地文件风险。
...
在账号模式下，模型列表、账号状态和用量信息由 MolaGPT 服务端提供。你可以像使用 Web 端一样选择模型，并在桌面端继续自己的对话工作流。

### BYOK 模式

BYOK 模式适合已经拥有第三方模型服务账号，或者希望接入自部署模型、代理服务、OpenRouter、New API、LiteLLM 等 OpenAI-compatible 服务的用户。

在 BYOK 模式下，请求会直接发送到你配置的服务端点。MolaGPT Desktop 只负责本地客户端、请求组装、流式响应解析和界面呈现。
...
Work 模式默认不会直接修改你的原始文件。

模型可以读取文件，但不能直接覆盖、删除或改写工作目录中的原始内容。生成的新文件、导出的结果和 Python 产生的中间产物，会进入独立的输出位置。

这样做是为了让 Desktop 版本拥有本地工作能力，同时避免高权限代理直接操作用户项目带来的风险。
